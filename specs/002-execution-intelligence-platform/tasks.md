...
**Purpose**: Implement secure dual-tier credential access and admin passcode management with strict UI invariance. ABSOLUTELY NO frontend menu, header, logo, or page layout alterations are permitted.

- [X] T200 [P] Implement Backend Auth Router in `backend/app/routers/auth.py` exposing `POST /api/auth/login` with Pydantic v2 validation (`LoginRequest`, `LoginResponse`). Enforce `@cipla.com` email domain gate (return `403 Forbidden` for all other domains except master admin), hardcoded Master Admin credentials (`adityaxnema@gmail.com` -> `"Guddan@1205"` with `isAdmin=True`), and default active shared passcode for `@cipla.com` users (`"AdityaIntern@2026"` with `isAdmin=True` if in designated admin whitelist: `pralhad.gujar@cipla.com`, `abhijeet.mudila@cipla.com`, `aditya.emmanual@cipla.com`, `adityaxnema@gmail.com`)
- [X] T201 [P] Implement Admin Password Change Endpoint `POST /api/auth/change-password` in `backend/app/routers/auth.py` with Pydantic v2 schema (`ChangePasswordRequest`). Enforce strict admin authorization (reject non-whitelisted emails with `403 Forbidden`), current passcode verification (`401 Unauthorized`), minimum 6-character length constraint, and Master Admin immutability protection (explicitly block any attempt targeting or updating `adityaxnema@gmail.com`'s credentials)
- [X] T202 Register Auth Router in `backend/app/routers/__init__.py` and `backend/app/main.py` using `app.include_router(auth.router)` to expose `/api/auth/login` and `/api/auth/change-password` endpoints alongside existing read services
- [X] T203 [P] Write Unit and Contract Tests in `backend/tests/api/test_auth.py` using pytest and FastAPI `TestClient` covering: (1) Master Admin login success with `"Guddan@1205"`, (2) Master Admin invalid password rejection (401), (3) Standard `@cipla.com` user login success with `"AdityaIntern@2026"`, (4) Whitelisted admin login success (`isAdmin=True`), (5) Unauthorized external domain rejection (403), (6) Admin password change and subsequent user authentication, and (7) Master Admin password immutability protection
- [X] T204 Transform ONLY static entrance button node in `frontend/src/App.tsx` (lines 230-238) into an inline glassmorphic Email + Passcode login form linking `onEnter` to `POST /api/auth/login`, while preserving 100% of header visual styling, Cipla logo badge (`CiplaLogoPlaceholder`), navigation dropdowns (`PRIMARY_PAGES`, `OPERATIONS_PAGES`), `UtilityMenu`, right-side decision scope panel, background effects, and grid cards. PROHIBIT any menu or header edits.
- [X] T205 Create automated PowerShell validation script `scripts/validate_auth.ps1` verifying end-to-end HTTP API contracts (`POST /api/auth/login`, `POST /api/auth/change-password`), domain gate enforcement, Master Admin immutability, and designated admin privilege validation against local backend server
